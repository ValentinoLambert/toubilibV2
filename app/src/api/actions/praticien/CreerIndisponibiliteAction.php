...
class CreerIndisponibiliteAction extends AbstractAction
{
    ...
    {
        $dto = $request->getAttribute(CreateIndisponibiliteMiddleware::ATTRIBUTE_DTO);
        if (!$dto instanceof InputIndisponibiliteDTO) {
        ...
        try {
            ...
            return $this->respondWithJson($response, $body, 201)
                ->withHeader('Location', '/praticiens/' . $dto->praticienId . '/indisponibilites/' . $indispo->id);
